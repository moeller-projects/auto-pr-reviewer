...
CRG enrichment: `CRG_ENABLED` (default `0`; set to `1` to activate the Tree-sitter context-enrichment stage — requires the `code-review-graph` package). When enabled, a knowledge graph is built from the checked-out working tree and change-impact analysis (changed functions, blast radius, test gaps, risk scores) is injected into the model prompt. Any failure degrades silently to today's behavior.
