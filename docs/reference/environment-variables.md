...
CRG enrichment: `CRG_ENABLED` (default `0`; set to `1` to activate the Tree-sitter context-enrichment stage — requires the `code-review-graph` package, installed in the container image). When enabled, a knowledge graph is built from the checked-out working tree and change-impact analysis (changed functions, blast radius, test gaps, risk scores) is injected into the model prompt. Any failure degrades silently to today's behavior. `CRG_CACHE_DIR` (optional) overrides the graph-cache root; it defaults to `REVIEW_ARTIFACT_ROOT/crg-cache` and container runs point it at the dedicated `reviewforge-crg-cache` volume mounted at `/workspace/crg-cache`. `CRG_CONTEXT_MAX_BYTES` (default `8192`) caps the graph-context block injected into the prompt.
