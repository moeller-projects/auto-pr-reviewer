...
def stage_context_files(ctx: Any, *, include_graph_context: bool = True) -> Path | None:
    ...
    existing_staging_dir = ctx.extras.get("context_staging_dir")
    owned_dir = bool(existing_staging_dir) and Path(existing_staging_dir) == staging_dir
    ...
    for name, source in sources.items():
        ...
        if not source.exists():
            destination.unlink(missing_ok=True)
            continue
        keys = _redacted_copy(source, destination, secrets)
        index[name] = {"description": _FILE_DESCRIPTIONS[name], "top_level_keys": keys}
